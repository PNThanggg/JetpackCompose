...
data class ArtistPage(
    ...
    fun filterExplicit(enabled: Boolean) = if (enabled) {
        copy(sections = sections.mapNotNull {
            it.copy(
                items = it.items.filterExplicit().ifEmpty {
    ...
    companion object {
        ...
        private fun fromMusicShelfRenderer(
                ...
                renderer.isAlbum -> {
                    AlbumItem(
                        browseId = renderer.navigationEndpoint.browseEndpoint?.browseId
                            ?: return null,
                        playlistId = renderer.thumbnailOverlay?.musicItemThumbnailOverlayRenderer?.content?.musicPlayButtonRenderer?.playNavigationEndpoint?.anyWatchEndpoint?.playlistId
                            ?: return null,
                        title = renderer.title.runs?.firstOrNull()?.text ?: return null,
                        artists = null,
