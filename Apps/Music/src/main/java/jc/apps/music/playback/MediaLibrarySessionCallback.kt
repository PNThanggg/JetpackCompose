...
import jc.apps.music.constants.MediaSessionConstants
...
import jc.apps.music.extensions.toggleRepeatMode
...
@Inject constructor(
    ...
    override fun onConnect(
        ...
        return MediaSession.ConnectionResult.accept(
            connectionResult.availableSessionCommands.buildUpon()
                ...
                .add(MediaSessionConstants.CommandToggleRepeatMode).build(),
    ...
    override fun onGetLibraryRoot(
        ...
        LibraryResult.ofItem(
            MediaItem.Builder().setMediaId(MusicService.ROOT).setMediaMetadata(
                MediaMetadata.Builder().setIsPlayable(false).setIsBrowsable(false)
            ...
            MusicService.PLAYLIST -> {
                ...
                listOf(
                    browsableMediaItem(
                        ...
                        context.resources.getQuantityString(
                            R.plurals.n_song, likedSongCount, likedSongCount
                        ),
                        ...
                        context.resources.getQuantityString(
                            R.plurals.n_song, downloadedSongCount, downloadedSongCount
                        ),
                ...
                parentId.startsWith("${MusicService.ARTIST}/") -> database.artistSongsByCreateDateAsc(
                    parentId.removePrefix("${MusicService.ARTIST}/")
                ).first().map {
                    it.toMediaItem(parentId)
                }

                parentId.startsWith("${MusicService.ALBUM}/") -> database.albumSongs(
                ...
                parentId.startsWith("${MusicService.PLAYLIST}/") -> when (val playlistId =
                    ...
                    PlaylistEntity.DOWNLOADED_PLAYLIST_ID -> {
                        ...
                        database.allSongs().flowOn(Dispatchers.IO).map { songs ->
                            ...
                            songs.map { it to downloads[it.id] }
                                .sortedBy { it.second?.updateTimeMs ?: 0L }.map { it.first }
                        }
                    }
        ...
        when (path.firstOrNull()) {
            ...
            MusicService.PLAYLIST -> {
                ...
                val songs = when (playlistId) {
                    PlaylistEntity.LIKED_PLAYLIST_ID -> database.likedSongs(
                        SongSortType.CREATE_DATE, descending = true
                    )
    ...
    private fun drawableUri(@DrawableRes id: Int) =
        Uri.Builder().scheme(ContentResolver.SCHEME_ANDROID_RESOURCE)
            .authority(context.resources.getResourcePackageName(id))
            .appendPath(context.resources.getResourceTypeName(id))
            .appendPath(context.resources.getResourceEntryName(id)).build()
    ...
    private fun browsableMediaItem(
        ...
        MediaMetadata.Builder().setTitle(title).setSubtitle(subtitle).setArtist(subtitle)
            .setArtworkUri(iconUri).setIsPlayable(false).setIsBrowsable(true)
            .setMediaType(mediaType).build()
    ).build()

    private fun Song.toMediaItem(path: String) = MediaItem.Builder().setMediaId("$path/$id")
        .setMediaMetadata(
            MediaMetadata.Builder().setTitle(song.title)
                .setSubtitle(artists.joinToString { it.name })
                .setArtist(artists.joinToString { it.name })
                .setArtworkUri(song.thumbnailUrl?.toUri()).setIsPlayable(true).setIsBrowsable(false)
