...
import jc.apps.music.constants.AlbumSortType
import jc.apps.music.constants.ArtistSongSortType
import jc.apps.music.constants.ArtistSortType
import jc.apps.music.constants.PlaylistSortType
import jc.apps.music.constants.SongSortType
import jc.apps.music.db.entities.Album
import jc.apps.music.db.entities.AlbumArtistMap
import jc.apps.music.db.entities.AlbumEntity
import jc.apps.music.db.entities.AlbumWithSongs
import jc.apps.music.db.entities.Artist
import jc.apps.music.db.entities.ArtistEntity
import jc.apps.music.db.entities.Event
import jc.apps.music.db.entities.EventWithSong
import jc.apps.music.db.entities.FormatEntity
import jc.apps.music.db.entities.LyricsEntity
import jc.apps.music.db.entities.Playlist
import jc.apps.music.db.entities.PlaylistEntity
import jc.apps.music.db.entities.PlaylistSong
import jc.apps.music.db.entities.PlaylistSongMap
import jc.apps.music.db.entities.RelatedSongMap
import jc.apps.music.db.entities.SearchHistory
import jc.apps.music.db.entities.Song
import jc.apps.music.db.entities.SongAlbumMap
import jc.apps.music.db.entities.SongArtistMap
import jc.apps.music.db.entities.SongEntity
import jc.apps.music.extensions.resize
import jc.apps.music.extensions.toSQLiteQuery
...
interface DatabaseDao {
    ...
    fun likedSongs(sortType: SongSortType, descending: Boolean) = when (sortType) {
    ...
    fun artists(sortType: ArtistSortType, descending: Boolean) = when (sortType) {
        ...
        artists.filter { it.artist.isYouTubeArtist }.reversed()
    }

    fun artistsBookmarked(sortType: ArtistSortType, descending: Boolean) = when (sortType) {
    ...
    fun albums(sortType: AlbumSortType, descending: Boolean) = when (sortType) {
    ...
    fun albumsLiked(sortType: AlbumSortType, descending: Boolean) = when (sortType) {
    ...
    fun playlists(sortType: PlaylistSortType, descending: Boolean) = when (sortType) {
    ...
    fun addSongToPlaylist(playlist: Playlist, songIds: List<String>) {
        ...
        songIds.forEach { id ->
            insert(
                PlaylistSongMap(
                    songId = id, playlistId = playlist.id, position = position++
                )
            )
        }
    }
    ...
    fun insert(mediaMetadata: MediaMetadata, block: (SongEntity) -> SongEntity = { it }) {
        ...
        mediaMetadata.artists.forEachIndexed { index, artist ->
            ...
            insert(
                SongArtistMap(
                    songId = mediaMetadata.id, artistId = artistId, position = index
                )
            )
        }
    }
    ...
    fun insert(albumPage: AlbumPage) {
        ...
        albumPage.songs.map(SongItem::toMediaMetadata).onEach(::insert).mapIndexed { index, song ->
            SongAlbumMap(
                songId = song.id, albumId = albumPage.album.browseId, index = index
            )
        }.forEach(::upsert)
        ...
        }?.onEach(::insert)?.mapIndexed { index, artist ->
            AlbumArtistMap(
                albumId = albumPage.album.browseId, artistId = artist.id, order = index
            )
        }?.forEach(::insert)
    }
    ...
    fun update(song: Song, mediaMetadata: MediaMetadata) {
        ...
        mediaMetadata.artists.forEachIndexed { index, artist ->
            ...
            insert(
                SongArtistMap(
                    songId = song.id, artistId = artistId, position = index
                )
            )
        }
    }
        ...
        albumPage.album.artists?.let { artists ->
            ...
            artists.map { artist ->
                ArtistEntity(
                    ...
                    ?: ArtistEntity.generateArtistId(), name = artist.name
                )
            }.onEach(::insert).mapIndexed { index, artist ->
                AlbumArtistMap(
                    albumId = albumPage.album.browseId, artistId = artist.id, order = index
