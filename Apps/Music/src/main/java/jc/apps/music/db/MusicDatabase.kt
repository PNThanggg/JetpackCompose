...
import jc.apps.music.db.entities.PlaylistSongMapPreview
...
import jc.apps.music.db.entities.SortedSongAlbumMap
import jc.apps.music.db.entities.SortedSongArtistMap
...
@Database(
    ...
    autoMigrations = [
        ...
        AutoMigration(
            from = 5, to = 6, spec = Migration5To6::class
        ),
        ...
        AutoMigration(
            from = 9, to = 10, spec = Migration9To10::class
        ),
        AutoMigration(from = 10, to = 11, spec = Migration10To11::class),
        AutoMigration(
            from = 11, to = 12, spec = Migration11To12::class
        ),
    ]
)
...
abstract class InternalDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        fun newInstance(context: Context): MusicDatabase = MusicDatabase(
    ...
    override fun migrate(database: SupportSQLiteDatabase) {
        ...
        database.query("SELECT * FROM artist".toSQLiteQuery()).use { cursor ->
            while (cursor.moveToNext()) {
                ...
                artists.add(
                    ArtistEntity(
                        id = newId, name = cursor.getString(1)
                    )
                )
            }
        }
        ...
        database.query("SELECT * FROM song".toSQLiteQuery()).use { cursor ->
            while (cursor.moveToNext()) {
                ...
                songArtistMaps.add(
                    SongArtistMap(
                        songId = songId, artistId = artistMap[cursor.getInt(2)]!!, position = 0
                    )
                )
            }
        }
...
@RenameColumn.Entries(
    RenameColumn(
        tableName = "song", fromColumnName = "download_state", toColumnName = "downloadState"
    ),
